...
class Category
{
    ...
    public static function getProductsByCategory($object, $data = array(), $only_filters = false)
    {
        ...
        $join .= '    
            ...
            LEFT JOIN dbi_postmeta ON  dbi_posts.id = dbi_postmeta.post_id  and dbi_postmeta.meta_key = "rait_prod"';

        if (isset($data['price_min'])) {
            $where .= " AND dbi_filters_values.price >= {$data['price_min']}";
        }
        if (isset($data['price_max'])) {
            $where .= " AND dbi_filters_values.price <= {$data['price_max']}";
        }
        if (isset($data['sales'])) {
            $where .= " AND dbi_filters_values.sales > 0";
        }
        ...
        $select = $only_filters ? 'dbi_filters_values.*' : 'dbi_posts.*, materials.name_' . $object['lang'] . ' as materials_name,brands.name_' . $object['lang'] . ' as brands_name, dbi_filters_values.*,if(dbi_postmeta.meta_value is null, 0 , dbi_postmeta.meta_value) as rait_prod';
