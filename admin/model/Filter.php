...
class Filter
{
    ...
    public static function getFilters($post_id,$type_name ='',$type_id = '')
    {
        ...
        if ($type_name !== '') {$sql .= " AND type_name = '{$type_name}'";}
        if ($type_id !== '' &&  $type_id !== null) {$sql .= " AND type_id = '{$type_id}'";}
    ...
    public static function updateFilter($data,$post_id,$type_name)
    {
        ...
        $db->query("UPDATE dbi_filters 
                        SET position = 0,   visible = false
                        WHERE post_id ={$post_id} and type_name = '{$type_name}'");

        foreach ($data as $d_c) {
            $db->query("UPDATE dbi_filters 
                        SET position = {$d_c->position},   visible = true
                        WHERE post_id ={$post_id} and type_id={$d_c->id} and type_name = '{$type_name}'");
        }
    }

    public static function updateFilterCheckbox($visible,$post_id,$type_name)
    {
        global $db;
    ...
    public static function removeFilter($type, $id)
    {
        ...
        $db->query("UPDATE dbi_posts SET brands_id = 0 WHERE brands_id = {$id}");
    }
