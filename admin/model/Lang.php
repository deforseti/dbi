...
class Lang
{
    ...
    public function createLangLocal()
    {
        ...
        $main_post = (int)$_POST['main_post'];
        $data_pages = $this->generateArrDataPageLocal();

        if ($langPagePostId = $this->check_post($main_post, $this->lang_arr['page_lang'])) {
            return $langPagePostId;
        }

        $langPagePostId = $this->createPageLang($data_pages);
        if (!$this->testCityLangInDb($main_post, $this->lang_arr['city_id'])) {
            $this->createLocal($langPagePostId, $main_post, $this->lang_arr['city_id']);
        }
        return $langPagePostId;
    }
    ...
    public function check_post($main_post, $lang)
    {
        global $db;
        return Db::returnResults($db->query('SELECT ' . $lang . ' as id FROM `langs` WHERE langs.main_post = ' . $main_post))['id'] ?? null;
    }
