				...
				if( is_array($object) ){
				 foreach ($object as $key => $post ) { ?>
					<li data-post-id="<?=$post['id']?>" data-post-position="<?=$post['position']?>" class="ui-state-default">
						...
						<a class="name_sortable_element" href="?page=singlpage&post_id=<?=$post['id']?>"><?=$post['post_name']?></a>
						<a class="pull-right link-post-opt redact-link-color" href="?page=singlpage&post_id=<?=$post['id']?>">Редактировать</a>
						<a onclick="return confirm('Удалить елемент?')" class="pull-right link-post-opt remove-link-color" href="?page=singlpage&remove_post=<?=$post['id']?>">Удалить</a>
